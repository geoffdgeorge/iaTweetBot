...
module.exports.iaTweetBot = async (event) => {
  ...
  const bookTitle = randomBook.title.length <= 183 ? randomBook.title : `${randomBook.title.substring(0, 179)}...`;
	...
	return T.post('media/upload', { media_data: base64Image }, function (
		err,
		data,
		response
	) {
		if (!err) {
			...
			return T.post('media/metadata/create', metaParams, function (
				err,
				data,
				response
			) {
				if (!err) {
					...
					return T.post('statuses/update', params, function (
						err,
						data,
						response
					) {
